...
class Buffer:
    ...
    def learn(self):
        ...
        with tf.GradientTape(persistent=True) as tape:
            ...
            critic_update_noise = tf.random.normal(
                target_actions.shape, mean=0, stddev=std_dev_critic_update)
            critic_update_noise = np.clip(
                critic_update_noise, -noise_clip, noise_clip)
            target_actions = target_actions.numpy()
            target_actions = np.clip(
                np.add(critic_update_noise, target_actions), lower_bound, upper_bound)
...
critic_update_noise = tf.random.normal(
    (64, 1), mean=0, stddev=std_dev_critic_update)
    ...
    for ep in range(total_episodes):
        ...
        while True:
            ...
            if update_toggle:
                ...
                update_target(target_critic_1.variables,
                              critic_model_1.variables, tau)
                update_target(target_critic_2.variables,
                              critic_model_2.variables, tau)
