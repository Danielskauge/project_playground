...
class Buffer:
    ...
    def update(self,state_batch, action_batch, reward_batch, next_state_batch):
        with tf.GradientTape(persistent=True) as tape:
            ...
            target_actions = np.clip(np.add(ou_noise_exploration(), target_actions),lower_bound,upper_bound)
...
#debug this
def get_target_actions(target_states, noise_object):
  target_actions = tf.squeeze(target_actor(target_states))
  target_actions = target_actions + np.clip(noise_object(), -noise_clip, noise_clip)
  target_actions = np.clip(target_actions, lower_bound, upper_bound)

  return [np.squeeze(target_actions)]

std_dev_exploration = 0.1
std_dev_critic_update = 0.1
...
def train():
    ...
    gamma = params['gamma']
    std_dev_exploration = params['std_dev_exploration']
    std_dev_critic_update = params['std_dev_critic_update']

    ou_noise_exploration = OUActionNoise(mean=np.zeros(
        1), std_deviation=float(std_dev_exploration) * np.ones(1))

    ou_noise_critic_update = OUActionNoise(mean=np.zeros(
